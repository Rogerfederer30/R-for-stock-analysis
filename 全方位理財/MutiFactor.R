...
for (i in 2:nrow(FinancialReportDate)){
  ## 選股
  ## 選股流程：1.成交量篩選：只挑交易日前一天的成交量大於1000張的股票。2.MA篩選：只挑交易日前一天的MA5>MA10的股票。3.ROE選股：ROE由大到小排序，挑選前5檔的股票投資。
  ...
  stock<-stock[-which(stock$trade_volume<1000),] # 刪除成交量小於1000張的股票
  # 計算各股票MA價格
  stocklist<- data.frame()
  for (ix in 1:nrow(stock)){
    date<-TradeDate[(min(which(TradeDate[,1]>=FinancialReportDate[i-1,1]))-9):min(which(TradeDate[,1]>=FinancialReportDate[i-1,1])),1]
    price<-StockPrice[which(StockPrice$code==stock[ix,1] & StockPrice$date %in% date),]
    # 挑選MA5>MA10的股票
    if (nrow(price)==10){
      if (mean(price[(nrow(price)-4):nrow(price),6])>mean(price[,6])) {
        stocklist<-rbind(stocklist, stock$code[ix])
      }
    }
  }
  colnames(stocklist)=c("code")
  # 投資ROE前5檔的股票
  stocklist<-FinancialReport[which(FinancialReport$year==FinancialReportDate[i-1,2] & FinancialReport$season==FinancialReportDate[i-1,3] & FinancialReport$code%in%stocklist$code),]
  ...
  for (ix in 1:nrow(stocklist)){
    InvestStock<-rbind(InvestStock,  cbind(buy_price=StockPrice$close[which(StockPrice$code==as.character(stocklist[ix,1]) & StockPrice$date==Date[1,1])],
                                          sell_price=StockPrice$close[which(StockPrice$code==as.character(stocklist[ix,1]) & StockPrice$date==Date[1,2])])) # 記錄個股買賣價格
  }
  InvestStock<-cbind(stocklist[,c(1:4)],InvestStock,return=InvestStock$sell_price/InvestStock$buy_price-1) # 記錄個股報酬
